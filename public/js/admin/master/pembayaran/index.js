$(document).ready(function () {
    const token = $("#pembayaranForm").data("token");
    const indexUrl = $("#pembayaranForm").data("index-url");
    const editUrl = $("#pembayaranForm").data("edit-url");
    const destroyUrl = $("#pembayaranForm").data("destroy-url");
    const successMessage = $("#pembayaranForm").data("success-message");
    const errorMessage = $("#pembayaranForm").data("error-message");
    $.ajaxSetup({
        headers: {
    ...
    function format(d) {
        ...
        <table>
            <tr>
                <td>Edit Data Pembayaran:</td>
                <td>
                    <a href="${editUrl}" class="btn btn-info">
                        <i class="fa fa-edit"></i> &nbsp; Edit Data Pembayaran
                    </a>
                </td>
            </tr>
            <tr>
                <td>Hapus Data Pembayaran:</td>
                <td>
                    <form action="${deleteUrl}" method="POST" class="deleteForm">
                        ...
                        <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah anda yakin untuk menghapus data ini ?')">
                            <i class="fa fa-times"></i> &nbsp; Hapus Data Pembayaran
                        </button>
                    </form>
                </td>
            </tr>
